...
func noteModel(t *testing.T, left, right []string, items ...notes.Note) *model {
	t.Helper()
	sidecar.Path, sidecar.Items = filepath.Join(t.TempDir(), "notes.json"), items
	t.Cleanup(func() { sidecar.Path, sidecar.Items = "", nil })
...
func TestNoteRowsPlacement(t *testing.T) {
	m := noteModel(t, []string{"a", "b", "c", "d"}, []string{"a", "B", "c", "d"},
		notes.Note{FilePath: "f.txt", NewLine: 3, Summary: "on c"},
		notes.Note{FilePath: "f.txt", Summary: "file level"},
		notes.Note{FilePath: "f.txt", OldLine: 2, Summary: "on old b"},
		notes.Note{FilePath: "other.txt", NewLine: 1, Summary: "elsewhere"},
	)
	if len(m.notes) != 3 {
...
func TestNotesShiftWithApplyAndUndo(t *testing.T) {
	m := noteModel(t, []string{"a", "b", "c", "d"}, []string{"a", "X", "Y", "c", "d"},
		notes.Note{FilePath: "f.txt", NewLine: 4, Summary: "on c"},
		notes.Note{FilePath: "f.txt", NewLine: 2, Summary: "inside the hunk"},
	)
...
func TestFoldKeepsNotedLine(t *testing.T) {
	...
	m := noteModel(t, left, right, notes.Note{FilePath: "f.txt", NewLine: 25, Summary: "deep in the fold"})
	for i, r := range m.rows {
		if r.note > 0 {
...
func TestVisualSkipsNoteRows(t *testing.T) {
	m := noteModel(t, []string{"a", "b", "c", "d", "e"}, []string{"a", "B", "C", "D", "e"},
		notes.Note{FilePath: "f.txt", NewLine: 2, Summary: "on B"})
...
func TestAddNoteWritesSidecarAndReloads(t *testing.T) {
	...
	if len(f.Comments) != 1 || f.Comments[0] != (notes.Note{FilePath: "f.txt", NewLine: 2, Summary: "use get_string here", Author: "human", Match: "B"}) {
	...
	f.Comments = append(f.Comments, notes.Note{FilePath: "f.txt", NewLine: 1, Summary: "agent reply"})
	data, _ = json.Marshal(f)
	os.WriteFile(sidecar.Path, data, 0o644)
	os.Chtimes(sidecar.Path, time.Now(), sidecar.Mtime().Add(2*time.Second))
	if !sidecar.Reload() || len(sidecar.Items) != 2 {
		t.Fatalf("reload failed: %d items", len(sidecar.Items))
	}
	m.loadFileNotes()
	m.recompute()
	if len(m.noteRows()) != 2 {
	...
	m.noteText = "use get_string here\nsecond line"
	m.saveNote()
	if sidecar.Items[0].Summary != "use get_string here\nsecond line" || len(sidecar.Items) != 2 {
		t.Fatalf("edit must rewrite in place: %+v", sidecar.Items)
	}
	// delete the note under the cursor
	m.deleteNote()
	if len(sidecar.Items) != 1 || len(m.noteRows()) != 1 {
		t.Fatalf("delete: %d items, %d rows", len(sidecar.Items), len(m.noteRows()))
	}
...
func TestNoteOnDeletionAnchorsOldLine(t *testing.T) {
	...
	m.saveNote()
	if n := sidecar.Items[0]; n.NewLine != 0 || n.OldLine != 2 {
...
func TestRangeNoteFromVisualAndShift(t *testing.T) {
	...
	m2 := noteModel(t, []string{"a", "b", "c", "d"}, []string{"a", "X", "Y", "c", "d"},
		notes.Note{FilePath: "f.txt", NewLine: 4, EndLine: 5, Summary: "c-d"})
	m2.apply(true)
...
func TestResolveToggle(t *testing.T) {
	m := noteModel(t, []string{"a", "b"}, []string{"a", "B"},
		notes.Note{FilePath: "f.txt", NewLine: 2, Summary: "check this\nsecond line"})
	m.gotoNote(1)
	...
	m.toggleResolved()
	n := m.notes[0]
	if !n.Resolved || len(m.noteLines(m.rows[m.curRow], 60, false)) != 1 || sidecar.Count("f.txt") != 0 {
		t.Fatalf("resolved: %+v lines=%d open=%d", n, len(m.noteLines(m.rows[m.curRow], 60, false)), sidecar.Count("f.txt"))
	}
	if m.noteAt(1, true) != nil {
		t.Fatal("resolved notes must not tint line numbers")
	}
	data, _ := os.ReadFile(sidecar.Path)
...
func TestNotesToggle(t *testing.T) {
	m := noteModel(t, []string{"a", "b", "c"}, []string{"a", "B", "c"},
		notes.Note{FilePath: "f.txt", NewLine: 2, Summary: "on B"}, notes.Note{FilePath: "f.txt", NewLine: 3, Summary: "on c"})
...
func TestMatchAnchorsWinOverLineNumbers(t *testing.T) {
	m := noteModel(t, []string{"a", "old only", "c"}, []string{"a", "b", "target line", "c", "b"},
		notes.Note{FilePath: "f.txt", NewLine: 1, Match: "target line", Summary: "moves to line 3"},
		notes.Note{FilePath: "f.txt", NewLine: 1, EndLine: 2, Match: "  target line ", Summary: "range keeps its length"},
		notes.Note{FilePath: "f.txt", NewLine: 5, Match: "b", Summary: "ambiguous: stays on 5"},
		notes.Note{FilePath: "f.txt", NewLine: 2, Match: "gone", Summary: "missing: stays on 2"},
		notes.Note{FilePath: "f.txt", NewLine: 2, Match: "old only", Summary: "found on the left only"},
	)
	got := []string{}
	...
	if lost := (&notes.Note{Match: "gone"}); noteTitle(lost) != "note · text not found" {
...
func TestHumanNoteRemembersLineText(t *testing.T) {
	...
	m.noteText = "here"
	m.saveNote()
	if sidecar.Items[0].Match != "unique text" {
		t.Fatalf("match = %q", sidecar.Items[0].Match)
	}
	...
	m2.noteText = "dup"
	m2.saveNote()
	if sidecar.Items[0].Match != "" || sidecar.Items[0].NewLine != 2 {
		t.Fatalf("ambiguous line must not record a match: %+v", sidecar.Items[0])
	}
}
